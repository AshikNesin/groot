...
async function getExternalDependencies() {
  try {
    ...
    console.error('Failed to read or parse package.json for external dependencies:', error);
    ...
    const fallbackExternals = [
        'express', '@t3-oss/env-core', 'cors',
        'dotenv', 'express-basic-auth', 'zod'
    ];
...
async function build() {
  ...
  try {
    await esbuild.build({
      ...
      platform: 'node',
      format: 'esm',
      target: 'esnext', // Or your specific Node version target if preferred
