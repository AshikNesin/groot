...
packages/                   # Boilerplate — always synced
  ...
  client/     @groot/shell # Client infrastructure (Layout, apiClient, stores, hooks)
  server/     @groot/core # Server infrastructure (core/ + shared/ via subpath exports)
  database/   @groot/core/database # Prisma client singleton + generated output
apps/web/prisma/            # schema.prisma + migrations — app-owned
...
```
apps/web → @groot/ui, @groot/shell, @groot/core, @groot/core/database
...
**Server imports:** `@groot/core/core/*` for infrastructure, `@groot/core/shared/*` for reusable features (auth, storage, jobs, etc.).

## Database (HOW)

The Prisma client lives in `@groot/core/database`. The generator output path in
`apps/web/prisma/schema.prisma` points to `packages/core/generated/prisma`
(gitignored, regenerated by `pnpm prisma generate`).

Import the singleton: `import { prisma } from "@groot/core/core/database"` or
directly from `@groot/core/database`.

Only `packages/core/src/database.ts` and `packages/core/src/client.ts`
...
- **TypeScript strict**.
- **Frontend imports**: `@groot/ui/*`, `@groot/shell/*`, relative `./app/*`.
- **Server imports**: `@groot/core/core/*`, `@groot/core/shared/*`, relative `./app/*`.
- **Frontend data hooks**: use `apiClient` from `@groot/shell/lib/api` (not raw axios).
- **Routes**: `createRouter()`; **controllers**: simple async fns.
- **Errors**: `Boom` factory methods from `@groot/core/core/errors`.
