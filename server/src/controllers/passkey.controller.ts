...
export class PasskeyController extends BaseController {
   ...
   * Requires authentication
   */
  verifyRegistration = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      if (!req.user) {
        ResponseHandler.error(res, 401, "Not authenticated");
        return;
   ...
   * Public endpoint (no authentication required)
   */
  verifyAuthentication = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const { response, email } = req.body;

      const { token, user } = await passkeyService.verifyAuthentication(
        response,
        email,
      );
   ...
   * Requires authentication
   */
  updatePasskeyName = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      if (!req.user) {
        ResponseHandler.error(res, 401, "Not authenticated");
        return;
