...
router.post("/bulk-rerun", async (req, res) => {
  try {
    ...
    return ResponseHandler.error(
      res,
      "Failed to bulk re-run jobs",
      "JOB_BULK_RERUN_ERROR",
    );
  }
});
...
router.delete("/state/:state", async (req, res) => {
  try {
    ...
    return ResponseHandler.success(
      res,
      { state: req.params.state, deletedCount },
      "Jobs purged",
    );
  } catch (error) {
...
router.get("/state/:state", async (req, res) => {
  try {
    ...
    return ResponseHandler.error(
      res,
      "Failed to get jobs by state",
      "JOB_STATE_LIST_ERROR",
    );
  }
});
