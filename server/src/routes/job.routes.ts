...
router.post("/", async (req, res) => {
  try {
    ...
    if (!jobName || !data) {
      return ResponseHandler.error(res, "Missing required fields: jobName and data", "JOB_VALIDATION_ERROR", 400);
    }
...
router.post("/schedule", async (req, res) => {
  try {
    ...
    if (!jobName || !data || !cron) {
      return ResponseHandler.error(res, "Missing required fields: jobName, data, cron", "JOB_SCHEDULE_VALIDATION_ERROR", 400);
    }
...
router.post("/:queueName/:jobId/retry", async (req, res) => {
  try {
    ...
    return ResponseHandler.success(res, { queueName, jobId }, `Job ${queueName}/${jobId} queued for retry`);
  } catch (error) {
...
router.post("/:queueName/:jobId/cancel", async (req, res) => {
  try {
    ...
    return ResponseHandler.success(res, { queueName, jobId }, `Job ${queueName}/${jobId} cancelled`);
  } catch (error) {
...
router.delete("/schedule/:jobName", async (req, res) => {
  try {
    ...
    return ResponseHandler.error(res, "Failed to cancel scheduled job", "JOB_SCHEDULE_CANCEL_ERROR");
  }
});
...
router.get("/", async (req, res) => {
  try {
    ...
    const limit = req.query.limit ? Number.parseInt(req.query.limit as string, 10) : 50;
    const offset = req.query.offset ? Number.parseInt(req.query.offset as string, 10) : 0;
    const jobs = await getJobs({ state, name, limit, offset });
