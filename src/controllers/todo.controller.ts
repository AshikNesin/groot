...
export const updateTodo = async (
  ...
  try {
    ...
    if (title === undefined && completed === undefined) {
      res
        .status(400)
        .json({ message: "No update data provided (title or completed)" });
      return;
    }
    ...
    if ((error as any).code === "P2025") {
      res
        .status(404)
        .json({ message: "Todo not found for update (Prisma P2025)" });
    } else {
...
export const deleteTodo = async (
  ...
  try {
    ...
    if ((error as any).code === "P2025") {
      res
        .status(404)
        .json({ message: "Todo not found for deletion (Prisma P2025)" });
    } else {
